...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 2 + i, Mouse.y + 2 + i, 1, 14 - (i * 2), Color(211, 211, 211, 255));
		Render::Clear(Mouse.x + 6, Mouse.y + 6, 1, 8, Color(211, 211, 211, 255));
		Render::Clear(Mouse.x + 7, Mouse.y + 7, 1, 9, Color(211, 211, 211, 255));
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 8 + i, Mouse.y + 8 + i, 1, 4 - i, Color(211, 211, 211, 255));
		Render::Clear(Mouse.x + 8, Mouse.y + 14, 1, 4, Color(211, 211, 211, 255));
		Render::Clear(Mouse.x + 9, Mouse.y + 16, 1, 2, Color(211, 211, 211, 255));
	}
}
...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	Render::DrawRectRainbow(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2 - 5, window->m_iWidth - 4 - 12 + 90, 7, 0.003f, rainbow);
	...
	Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2 - 10, window->m_iWidth - 4 - 12 + 96, 6, Color(40, 40, 40, MenuAlpha));
	...
	Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2 - 6, 6, window->m_iHeight - 2 - 8 - 26 + 12, Color(40, 40, 40, MenuAlpha));
...
//	Render::GradientSideways((window->m_x + window->m_iWidth / 2) - 81, window->m_y + 58, window->m_iWidth / 2 + 71, 1, Color(160, 0, 255, MenuAlpha), Color(255, 255, 0, MenuAlpha), 1);
	...
	if (tabcount) // If there are some tabs
	{
		...
		for (int i = 0; i < tabcount; i++)
		{
			...
			RECT TabDrawArea = { window->m_x + 8 - 80 + 1, yAxis - 5, 90 - 1, intercept };
